...
**Note**: Gurobi seems to hang for when calling the solver, which causes longer times than are reported in the paper (usually adds about 100 or so seconds to the total runtime). This may go away if you obtain the academic license, or if you use a bare-metal machine. We are not sure why this is happening.
...
# Code Documentation
The code for our work has two key components: (1) the LinSyn procedure described in the paper, (2) the auto_lirpa framework which we build on top of. 
